...
import React, { ToastAndroid, AsyncStorage, Component, ListView, View, ProgressBarAndroid, StyleSheet, Image, Text, PropTypes } from 'react-native';
...
export default class ProfileView extends Component {
  ...
  getData(data) {
    ...
    Server.get('/monthly_summary.json', params)
    .then((data) => {
      const monthlySummary = data.user.monthly_summary;
      const totalDuration = monthlySummary.reduce((prev, curr) => { return { duration: prev.duration + curr.duration };} ).duration;
      const totalAmount = monthlySummary.reduce((prev, curr) => { return { amount_contributed: prev.amount_contributed + curr.amount_contributed };} ).amount_contributed;
      this.setState({
        isLoading: false,
        totalDuration,
        totalAmount,
    ...
    if(this.state.isLoading) {
      ...
      return (
        <View style={styles.container}>
          <Image source={{ uri: 'http://move1t.herokuapp.com/img/profile_background02.jpg' }}
            style={styles.background}
          >
            <Image source={{ uri: `${this.state.user.gravatar}&s=250&d=mm` }}
              style={styles.thumb}
            />
          ...
          <View style={{ alignItems: 'center' }}>
            <Dropdown onChange={(data) => this.getData(data)}
              selected={this.state.selected}
              style={{ height: 20, width: 120 }}
              values={this.renderMonthList()}
            />
          </View>
          <ListView dataSource={this.entryList()}
